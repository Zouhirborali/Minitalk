...
/*   Updated: 2024/05/10 15:24:00 by zbakkas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_sign(siginfo_t *info, char *str, int *i, char **print)
{
...
void	handler(int signum, siginfo_t *info, void *con)
{
	...
	if (signum == SIGUSR1)
		...
		str[x++] = '0';
	str[x] = 0;
	if ((i != 0 && (ft_strlen(print) == i || ft_strlen(str) == 8))
		|| (ft_strlen(str) == 32 && i == 0))
		x = 0;
	get_sign(info, str, &i, &print);
}
